...
protected:
    ...
    enum {BINARY, UNARY,COND, UNCOND, RET, LOAD, STORE, CMP, ALLOCA, FUNCCALL,EXT};
};
...
{
public:
    GlobalAllocaInstruction(Operand *dst, SymbolEntry *se, std::string val="0",BasicBlock *insert_bb = nullptr);
...
{
public:
    FuncCallInstruction(Operand *dst,SymbolEntry* se, std::vector<Operand*> params, BasicBlock *insert_bb =nullptr);
    ~FuncCallInstruction();
    void output() const;
    void genMachineCode(AsmBuilder*);
private:
